...
def load_rf_wav(path):
    """
    Load RF traces from a WAV file.
    
    Parameters:
    - path: Input WAV file path
    
    Returns:
    - traces: Array of RF data
    - fs: Sampling frequency in Hz
    """
...
def slice_traces(data, fs, duration, num_traces, offset=0):
    """
    Slice continuous data into fixed-length traces.
    
    Parameters:
    - data: Continuous data (raw or envelope)
    - fs: Sampling frequency in Hz
    - duration: Duration of each trace in seconds
    - num_traces: Number of traces to extract
    - offset: Starting offset in samples
    
    Returns:
    - sliced_traces: Array of shape (num_traces, samples_per_trace)
    """
    samples_per_trace = int(duration * fs)
    total_samples = len(data)
    
    # Check if there's enough data
    required_samples = offset + num_traces * samples_per_trace
    if required_samples > total_samples:
        raise ValueError(f"Not enough data. Required: {required_samples}, Available: {total_samples}")
    
    sliced_traces = np.zeros((num_traces, samples_per_trace))
    
    for i in range(num_traces):
        start_idx = offset + i * samples_per_trace
        end_idx = start_idx + samples_per_trace
        sliced_traces[i] = data[start_idx:end_idx]
    
    return sliced_traces

def load_public_dataset(file_path, trace_offset=0, num_traces=1000):
    """
    Load traces from a public dataset (ASCAD or SPERO) in HDF5 format with improved error handling.
    
    Parameters:
    - file_path: Path to the HDF5 file
    - trace_offset: Offset from the beginning of the dataset
    - num_traces: Number of traces to load
    
    Returns:
    - traces: Array of traces
    - plaintexts: Array of plaintexts
    - key: True key bytes from the dataset (if available)
    
    Raises:
    - FileNotFoundError: If the file does not exist
    - ValueError: If the file format is invalid or required data is missing
    - TypeError: If the data types are incompatible with analysis
    """
    import numpy as np
    import h5py
    import os
    ...
    if not file_path.lower().endswith(('.h5', '.hdf5')):
        raise ValueError(f"File does not have a valid HDF5 extension: {file_path}")
    
    try:
        with h5py.File(file_path, 'r') as f:
            # First, inspect file structure to detect format
            ...
            # Specifically check for ASCAD format first
            if 'Attack_traces/traces' in f:
                print("Detected ASCAD attack traces format")
                # ASCAD attack traces format
                max_available = len(f['Attack_traces/traces'])
                if trace_offset >= max_available:
                    raise ValueError(f"Trace offset {trace_offset} exceeds available traces ({max_available})")
                
                num_to_load = min(num_traces, max_available - trace_offset)
                print(f"Loading {num_to_load} traces from offset {trace_offset}")
                
                traces = f['Attack_traces/traces'][trace_offset:trace_offset+num_to_load]
                
                # Extract plaintexts - handle structured arrays carefully
                plaintexts = np.zeros((num_to_load, 16), dtype=np.uint8)
                metadata = f['Attack_traces/metadata'][trace_offset:trace_offset+num_to_load]
                
                for i in range(num_to_load):
                    # Extract plaintext from metadata as array
                    if isinstance(metadata[i]['plaintext'], np.ndarray):
                        plaintexts[i] = metadata[i]['plaintext']
                    else:
                        # If it's a single value (might be structured)
                        pt_data = metadata[i]['plaintext']
                        if isinstance(pt_data, np.void):
                            plaintexts[i, 0] = pt_data[0]  # Just use first byte if structured
                        else:
                            plaintexts[i, 0] = pt_data
                
                # Extract key if available
                try:
                    key_data = metadata[0]['key']
                    if isinstance(key_data, np.ndarray):
                        key = key_data.astype(np.uint8)
                    elif isinstance(key_data, np.void):
                        key = np.array([key_data[0]], dtype=np.uint8)
                    else:
                        key = np.array([key_data], dtype=np.uint8)
                    print(f"Successfully extracted key: {key.tobytes().hex()}")
                except Exception as e:
                    print(f"Warning: Could not extract key: {e}")
                    key = None
            
            # Try profiling traces if attack traces not found
            elif 'Profiling_traces/traces' in f:
                print("Detected ASCAD profiling traces format")
                # ASCAD profiling traces format
                max_available = len(f['Profiling_traces/traces'])
                if trace_offset >= max_available:
                    raise ValueError(f"Trace offset {trace_offset} exceeds available traces ({max_available})")
                ...
                traces = f['Profiling_traces/traces'][trace_offset:trace_offset+num_to_load]
                
                # Extract plaintexts - handle structured arrays carefully
                plaintexts = np.zeros((num_to_load, 16), dtype=np.uint8)
                metadata = f['Profiling_traces/metadata'][trace_offset:trace_offset+num_to_load]
                ...
                for i in range(num_to_load):
                    ...
                    if 'plaintext' in metadata.dtype.names:
                        ...
                        if isinstance(pt_data, np.ndarray):
                            plaintexts[i, :len(pt_data)] = pt_data
                        elif isinstance(pt_data, np.void):
                            # Handle structured type - extract the first byte or element
                            plaintexts[i, 0] = pt_data[0]
                        else:
                            # Single value
                            plaintexts[i, 0] = pt_data
            ...
            # Check for SPERO format next
            elif 'traces' in f and 'metadata' in f:
                print("Detected SPERO format dataset")
                # SPERO format
                max_available = len(f['traces'])
                if trace_offset >= max_available:
                    raise ValueError(f"Trace offset {trace_offset} exceeds available traces ({max_available})")
                ...
                traces = f['traces'][trace_offset:trace_offset+num_to_load]
                
                if 'plaintext' not in f['metadata']:
                    raise ValueError("Missing plaintext data in SPERO format dataset")
                
                # Handle possible structured arrays for plaintexts
                plaintexts_data = f['metadata']['plaintext'][trace_offset:trace_offset+num_to_load]
                
                # Create target plaintext array
                plaintexts = np.zeros((num_to_load, 16), dtype=np.uint8)
                
                # Handle different possible plaintext formats
                try:
                    for i in range(num_to_load):
                        if isinstance(plaintexts_data[i], np.ndarray):
                            # If already an array, copy as much as will fit
                            plaintexts[i, :len(plaintexts_data[i])] = plaintexts_data[i][:16]
                        elif isinstance(plaintexts_data[i], np.void):
                            # If structured type, convert to bytes
                            plaintext_bytes = np.frombuffer(plaintexts_data[i].tobytes(), dtype=np.uint8)
                            plaintexts[i, :len(plaintext_bytes)] = plaintext_bytes[:16]
                        else:
                            # Single value
                            plaintexts[i, 0] = plaintexts_data[i]
                except Exception as e:
                    raise ValueError(f"Error processing plaintexts: {e}")
                
                # Extract key if available
                if 'key' in f['metadata']:
                    try:
                        key_data = f['metadata']['key'][0]
                        # Convert to standard byte array if it's a structured type
                        if isinstance(key_data, np.void):
                            key = np.frombuffer(key_data.tobytes(), dtype=np.uint8)
                        else:
                            key = np.array(key_data, dtype=np.uint8)
                ...
                print("Unknown dataset format, attempting generic detection...")
                trace_keys = [k for k in f.keys() if 'trace' in k.lower()]
                plaintext_keys = [k for k in f.keys() if 'plain' in k.lower()]
                key_keys = [k for k in f.keys() if 'key' in k.lower()]
                
                if not trace_keys:
                    raise ValueError("Could not find trace data in dataset (no keys with 'trace' in the name)")
                
                if not plaintext_keys:
                    raise ValueError("Could not find plaintext data in dataset (no keys with 'plain' in the name)")
                
                print(f"Using trace data from key: {trace_keys[0]}")
                print(f"Using plaintext data from key: {plaintext_keys[0]}")
                
                max_available = len(f[trace_keys[0]])
                if trace_offset >= max_available:
                    raise ValueError(f"Trace offset {trace_offset} exceeds available traces ({max_available})")
                ...
                traces = f[trace_keys[0]][trace_offset:trace_offset+num_to_load]
                plaintexts_raw = f[plaintext_keys[0]][trace_offset:trace_offset+num_to_load]
                
                # Create target plaintext array and attempt to convert
                plaintexts = np.zeros((num_to_load, 16), dtype=np.uint8)
                
                # Handle various plaintext formats
                for i in range(num_to_load):
                    pt_data = plaintexts_raw[i]
                    if isinstance(pt_data, np.ndarray):
                        plaintexts[i, :min(16, len(pt_data))] = pt_data[:16]
                    elif isinstance(pt_data, np.void):
                        # For structured arrays, try converting to bytes
                        try:
                            pt_bytes = np.frombuffer(pt_data.tobytes(), dtype=np.uint8)
                            plaintexts[i, :min(16, len(pt_bytes))] = pt_bytes[:16]
                        except:
                            # If that fails, try extracting individual fields
                            if hasattr(pt_data, 'dtype') and hasattr(pt_data.dtype, 'names'):
                                field = pt_data.dtype.names[0]  # Use first field
                                plaintexts[i, 0] = pt_data[field]
                    else:
                        # Single value
                        plaintexts[i, 0] = pt_data
                ...
                if key_keys:
                    try:
                        print(f"Using key data from key: {key_keys[0]}")
                        key_data = f[key_keys[0]][0]
                        # Convert to standard byte array if it's a structured type
                        if isinstance(key_data, np.void):
                            key = np.frombuffer(key_data.tobytes(), dtype=np.uint8)
                        else:
        ...
        if traces is None or len(traces) == 0:
            raise ValueError("No trace data was loaded")
            
        print(f"Loaded traces shape: {traces.shape}")
        
        # Ensure traces are a 2D array
        if traces.ndim != 2:
            raise ValueError(f"Expected 2D array for traces, got {traces.ndim}D array")
            
        # Ensure plaintexts exist
        if plaintexts is None or len(plaintexts) == 0:
            raise ValueError("No plaintext data was loaded")
...
# ─── PURE SDR REAL‑WORLD DATA LOADER ─────────────────────────────────────────
def load_real_sdr_dataset(path, dataset="screaming_channels", offset=0, count=500):
    """
    Load real SDR I/Q traces from one of three public datasets:
      • screaming_channels: HDF5 of BLE side‑channel captures
      • drone_rf_video: raw .bin I/Q (int16 interleaved)
      • migou_mod: pickled .pkl files containing I/Q arrays
    Returns: traces (num_traces×samples), plaintexts (random), key (None)
    """
    import glob, h5py, numpy as np, pickle, os, scipy.io.wavfile as wav

    traces = None

    if dataset == "screaming_channels":
        files = glob.glob(os.path.join(path, "*.h5"))
        if not files:
            raise FileNotFoundError("No .h5 files in screaming_channels folder")
        with h5py.File(files[0], "r") as f:
            # adjust the group name if needed
            traces = f["attack_traces/traces"][offset:offset+count]

    elif dataset == "drone_rf_video":
        # .bin files with int16 interleaved I/Q
        bins = glob.glob(os.path.join(path, "*.bin"))
        if not bins:
            raise FileNotFoundError("No .bin files in drone_rf_video folder")
        raw = np.fromfile(bins[0], dtype=np.int16)
        # split into complex64: real=even, imag=odd
        iq = raw.reshape(-1, 2)
        complex_samples = iq[:,0].astype(np.float32) + 1j*iq[:,1].astype(np.float32)
        # slice into `count` traces
        L = len(complex_samples)//count
        traces = complex_samples[:L*count].reshape(count, L)

    elif dataset == "migou_mod":
        # .pkl files with pickled dicts; assume each has key 'iq'→np.ndarray
        pkls = glob.glob(os.path.join(path, "*.pkl"))
        if not pkls:
            raise FileNotFoundError("No .pkl files in migou_mod folder")
        with open(pkls[0], "rb") as f:
            data = pickle.load(f)
        # expect data['iq'] to be shape (N_traces, samples) or (samples,)
        raw = data.get("iq", None) or data.get("traces", None)
        if raw is None:
            raise ValueError("Pickle does not contain 'iq' or 'traces' key")
        arr = np.array(raw)
        # if 1D split into count traces, else take first count
        if arr.ndim == 1:
            L = len(arr)//count
            traces = arr[:L*count].reshape(count, L)
        else:
            traces = arr[offset:offset+count]

    else:
        raise ValueError(f"Unknown dataset: {dataset}")

    # we have I/Q or float traces; convert to real envelopes for CPA
    # if complex, take magnitude
    if np.iscomplexobj(traces):
        traces = np.abs(traces)

    # generate dummy plaintexts and no key
    num, samples = traces.shape
    plaintexts = np.random.randint(0, 256, (num, 16), dtype=np.uint8)
    key = None
